...
ball_colliding_arena(Ball *ball) {
	if (ball->desired_p.x + ball->half_size.x > arena.right_wall_visual_p) {
		...
		spawn_particle_explosion(10, ball->desired_p, 8.f, 1.f, .15f, arena.wall_color);
	}
...
ball_colliding_player(Ball *ball) {
	if (ball->dp.y < 0 && is_colliding(player.visual_p, player.half_size, ball->desired_p, ball->half_size)) {
		increase_ball_size(ball);
		...
		spawn_particle_explosion(10, ball->desired_p, 8.f, 1.f, .15f, player.color);
